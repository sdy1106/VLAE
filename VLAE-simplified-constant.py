...
parser.add_argument('--result_path', type=str, default='./result/VLAE_constant')
...
def q_net(observed, x, is_training):
    with zs.BayesianNet(observed=observed) as encoder:
        ...
        latent1_mean = layers.fully_connected(ladder1, font_dim, activation_fn=tf.identity)
        latent1_std = layers.fully_connected(ladder1, font_dim, activation_fn=tf.sigmoid)
        ...
        latent2_mean = layers.fully_connected(ladder2, char_dim, activation_fn=tf.identity)
        latent2_std = layers.fully_connected(ladder2, char_dim, activation_fn=tf.sigmoid)

        z_char = zs.Normal('z_char', mean=latent2_mean, std=latent2_std, n_samples=1, group_event_ndims=1)
        z_font = zs.Normal('z_font', mean=latent1_mean, std=latent1_std, n_samples=1, group_event_ndims=1)
...
def main():
    if args.dataset == 'standard':
        X = np.load('/home/danyang/mfs/data/hccr/image_1000x200x64x64_stand.npy')
    ...
    pairwise_alpha = tf.placeholder(tf.float32 , shape=[] , name='pairwise_alpha')
    ...
    def build_tower_graph(id_):
        ...
        qz_samples_font_source, log_qz_font_source = encoder.query('z_font', outputs=True,
                                                     local_log_prob=True)
        encoder, _, _ = q_net(None, tower_char_source, is_training)
        qz_samples_char_source, log_qz_char_source = encoder.query('z_char', outputs=True,
                                                     local_log_prob=True)

        _, x_recon = VLAE({'z_font': qz_samples_font_source, 'z_char': qz_samples_char_source}, tf.shape(qz_samples_char_source)[0], is_training)

        encoder, _, _ = q_net( {'z_font':qz_samples_font_source , 'z_char':qz_samples_char_source} , x_recon, is_training)
        _, log_qz_font_recon =  encoder.query('z_font', outputs=True,
                                                     local_log_prob=True)
        _, log_qz_char_recon = encoder.query('z_char', outputs=True,
                                                   local_log_prob=True)
        constant_loss = tf.reduce_mean(log_qz_font_source-log_qz_font_recon) + tf.reduce_mean(log_qz_char_source-log_qz_char_recon)
        ...
        lower_bound_pairwise = pairwise_alpha * tf.reduce_mean(
            zs.iwae(log_joint, {'x': x_obs},
                    {'z_font': [qz_samples_font_source, log_qz_font_source], 'z_char': [qz_samples_char_source, log_qz_char_source]}, axis=0))

        grads = optimizer.compute_gradients(-lower_bound-lower_bound_pairwise + constant_loss)
        return grads, [lower_bound, lower_bound_pairwise , constant_loss]
    ...
    lower_bound, lower_bound_pairwise , constant_loss = multi_gpu.average_losses(tower_losses)
    ...
    with multi_gpu.create_session() as sess:
        ...
        for epoch in range(begin_epoch, epochs + 1):
            ...
            lower_bounds, lower_bounds_pairwise , constant_losses = [], [] , []
            ...
            for i in range(train_iters):
                if args.pairwise:
                    _, lb, lbp , cl = sess.run(
                        [infer, lower_bound, lower_bound_pairwise , constant_loss],
                ...
                constant_losses.append(cl)
            print('Epoch={} ({:.3f}s/epoch): '
                  'Lower Bound = {} Lower Bound Pairwise = {} Constant loss = {}'.
                  format(epoch, (time.time() + time_train),
                         np.mean(lower_bounds), np.mean(lower_bounds_pairwise) , np.mean(constant_losses)))
