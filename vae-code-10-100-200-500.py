...
def build_table():
    ...
    bin_table.sort(comp)
    return bin_table
bin_table = build_table()
dense_label = np.zeros((n_y , sample_num , 10))
for i in range(n_y):
    for j in range(sample_num):
        for k in range(len(bin_table[i])):
            if bin_table[i][k] == 1:
                dense_label[i , j , k] = 1.0
...
if __name__ == "__main__":
    ...
    if not args.code:
        if args.dataset == 'hand':
            ...
            if args.dense:
                t_train , t_test = dense_label[:n_y , :int(sample_num*train_test_rate[0]) , :] , dense_label[:n_y , :int(sample_num*train_test_rate[1]) , :]
                n_code = 10
            else:
                t_train, t_test = \
                    utils.to_onehot(t_train, n_y), utils.to_onehot(t_test, n_y)
