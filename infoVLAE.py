...
def main():
	...
	lower_bound, info_char, info_font = multi_gpu.average_losses(tower_losses)
	...
	with multi_gpu.create_session() as sess:
		...
		for epoch in range(begin_epoch, epochs + 1):
			...
			lower_bounds, infos_char, infos_font = [], [], []
			...
			for i in range(train_iters):
				if args.pairwise:
					_, lb, Ic, If = sess.run(
						[infer, lower_bound, info_char, info_font],
				...
				infos_char.append(Ic)
				infos_font.append(If)
			print('Epoch={} ({:.3f}s/epoch): '
				  'Lower Bound = {} Infomation Character = {}, Information Font = {}'.
				  format(epoch, (time.time() + time_train),
						 np.mean(lower_bounds), np.mean(infos_char), np.mean(infos_font)))
