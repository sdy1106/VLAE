...
parser.add_argument('--result_path', type=str, default='./result/iwae-64-standard-test-1')
...
if __name__ == "__main__":
    ...
    for i in range(display_each_character):
        char_index = 15 #random.randint(0, n_y - 1)
    ...
    x_train_interp = np.concatenate((x_train[:test_ny, 41, :, :], x_train[:test_ny, 5, :, :]), axis=1)
    t_train_interp = np.concatenate((t_train[:test_ny, 41, :], t_train[:test_ny, 5, :]), axis=1)
