...
def VLAE(observed, n, is_training):
    with zs.BayesianNet(observed=observed) as decoder:
        ...
        ladder2 = layers.fully_connected(latent2, ngf * 8 * 4 * 4, activation_fn=tf.nn.relu,
                                         ...
                                         normalizer_params=normalizer_params,
                                         weights_initializer=init_ops.RandomNormal(stddev=0.02))
        ladder2 = tf.reshape(ladder2, [-1, 2, 2, ngf * 8])
        ...
        ladder1 = layers.fully_connected(latent1, ngf * 4 * 8 * 8, activation_fn=tf.nn.relu,
                                         ...
                                         normalizer_params=normalizer_params,
                                         weights_initializer=init_ops.RandomNormal(stddev=0.02))
        ladder1 = tf.reshape(ladder1, [-1, 4, 4, ngf * 4])
        ...
        x_logits = layers.conv2d_transpose(ladder0, 3, 4, stride=2, activation_fn=tf.identity,
                                           weights_initializer=init_ops.RandomNormal(stddev=0.02))

        x_logits = tf.reshape(x_logits, [1, -1, n_x])
        x = zs.Normal('x', x_logits, n_samples=1, group_event_ndims=1)
...
def main():
    ...
    train_x = sio.loadmat('train_32x32.mat')['X'].astype(np.float32)
    test_x = sio.loadmat('test_32x32.mat')['X'].astype(np.float32)
    ...
    def build_tower_graph(id_):
        ...
        grads = optimizer.compute_gradients(-lower_bound-lower_bound_pairwise)
    ...
    with multi_gpu.create_session() as sess:
        ...
        for epoch in range(begin_epoch, epochs + 1):
            ...
            print('Epoch={} ({:.3f}s/epoch): '
                  'Lower Bound = {} Lower Bound Pairwise = {}'.
                  format(epoch, (time.time() + time_train),
                         np.mean(lower_bounds), np.mean(lower_bounds_pairwise)))
