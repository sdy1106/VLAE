...
parser.add_argument('--char_dim', type=int, default=40)
...
def main():
    if args.dataset == 'standard':
        X = np.load('/home/danyang/mfs/data/hccr/image_2000x163x64x64_stand.npy')
        if n_font > 100:
    ...
    def build_tower_graph(id_):
        ...
        qz_samples_font_source, log_qz_font_source = encoder.query('z_font', outputs=True, local_log_prob=True)
        encoder, _, _ = q_net(None, tower_char_source, is_training)
        qz_samples_char_source, log_qz_char_source = encoder.query('z_char', outputs=True, local_log_prob=True)

        encoder , _ , _ = q_net({'z_font':qz_samples_font_source , 'z_char':qz_samples_char_source} , x_obs , is_training)
        qz_font_obs , log_qz_font_obs = encoder.query('z_font' , outputs=True , local_log_prob=True)
        qz_char_obs , log_qz_char_obs = encoder.query('z_char' , outputs=True , local_log_prob=True)

        KL_font = tf.reduce_mean(log_qz_font_source - log_qz_font_obs)
        KL_char = tf.reduce_mean(log_qz_char_source - log_qz_char_obs)

        constrain = (KL_char + KL_font)

        lower_bound = tf.reduce_mean(zs.iwae(log_joint, {'x': x_obs}, {'z_font': [qz_samples_font, log_qz_font],
                                                                       'z_char': [qz_samples_char, log_qz_char]},
                                             axis=0))

        lower_bound_pairwise = tf.reduce_mean(
            zs.iwae(log_joint, {'x': x_obs},
                    {'z_font': [qz_samples_font_source, log_qz_font_source],
                     'z_char': [qz_samples_char_source, log_qz_char_source]}, axis=0))

        grads = optimizer.compute_gradients(-lower_bound - 0.01*lower_bound_pairwise + constrain )
        return grads, [lower_bound, lower_bound_pairwise , constrain]
    ...
    lower_bound, lower_bound_pairwise , constrain = multi_gpu.average_losses(tower_losses)
    ...
    with multi_gpu.create_session() as sess:
        ...
        for epoch in range(begin_epoch, epochs + 1):
            ...
            lower_bounds, lower_bounds_pairwise , constrains = [], [] ,[]
            ...
            for i in range(train_iters):
                if args.pairwise:
                    _, lb, lbp , cs = sess.run(
                        [infer, lower_bound, lower_bound_pairwise , constrain],
            ...
            print('Epoch={} ({:.3f}s/epoch): '
                  'Lower Bound = {} Lower Bound Pairwise = {} Constrain = {}'.
                  format(epoch, (time.time() + time_train),
                         np.mean(lower_bounds), np.mean(lower_bounds_pairwise) , np.mean(constrains)))
            ...
            gen_images = sess.run(x_gen,
                                  feed_dict={
                                      char_source: test_x[char_index[:10], :, :, :][:, font_index[:10], :, :].reshape(
                                          -1, n_x),
                                      font_source: test_x[char_index[:10], :, :, :][:, font_index[:10], :, :].reshape(
                                          -1, n_x),
                                      is_training: False})
