...
def q_net(observed, x, is_training):
	with zs.BayesianNet(observed=observed) as encoder:
		...
		latent1_mean = layers.fully_connected(ladder1, char_dim / 2, activation_fn=tf.identity)
		latent1_std = layers.fully_connected(ladder1, char_dim / 2, activation_fn=tf.sigmoid)
		...
		mu_char = tf.concat([latent1_mean, latent2_mean], 1)
		std_char = tf.concat([latent1_std, latent2_std], 1)
		z_char = zs.Normal('z_char', mean=mu_char, std=std_char, n_samples=1, group_event_ndims=1)
		z_font = zs.Normal('z_font', mean=latent0_mean, std=latent0_std, n_samples=1, group_event_ndims=1)
